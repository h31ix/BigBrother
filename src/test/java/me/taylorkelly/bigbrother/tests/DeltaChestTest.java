...
public class DeltaChestTest {
    ...
    public void testDelta() {
        ...
        stackOrig = new ItemStack[] { new ItemStack(1, 15), new ItemStack(1, 16), new ItemStack(1, 64), new ItemStack(1, 64), new ItemStack(0) };
        // Same as above, EXCEPT...
        stackNew = new ItemStack[] { new ItemStack(1, 16), // added 1
     ...
     */
    /*
     * @Test public void testRollback() { // Create chest Block blockA = world.getBlockAt(0,0,0); blockA.setTypeId(Material.CHEST.getId()); Chest c = (Chest)blockA.getState();
     * 
     * // Clear any leftovers c.getInventory().clear();
     * 
     * // Add 64 gold ingots. c.getInventory().addItem(new ItemStack(Material.GOLD_INGOT.getId(),64)); stateA=c.getInventory().getContents();
     * 
     * //Create fake player. douchebag = new BBPlayerInfo(1, "Douchebag", 1); // ID #1: Douchebag (Watched)
     * 
     * // Simulate player stealing gold. douchebag.setHasOpenedChest(c, stateA); c.getInventory().clear();
     * 
     * // Record theft World world = douchebag.getOpenedChest().getWorld(); int x = douchebag.getOpenedChest().getX(); int y = douchebag.getOpenedChest().getY(); int z = douchebag.getOpenedChest().getZ(); Chest chest = (Chest)world.getBlockAt(x, y, z).getState(); ItemStack[] orig = douchebag.getOldChestContents(); ItemStack[] latest = chest.getInventory().getContents(); DeltaChest dc = new DeltaChest(douchebag.getName(), chest, orig, latest); //dc.send();
     * 
     * dc.rollback(world);
     * 
