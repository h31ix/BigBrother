...
public class BBDataH2 extends BBDataTable {
    ...
    public int getCleanseByLimit(Statement stmt, Long maxRecords,
        ...
        String cleansql = "DELETE FROM `" + getTableName() + "` LEFT OUTER JOIN (SELECT `id` FROM `" + getTableName() + "` ORDER BY `id` DESC LIMIT 0," + maxRecords + ") AS `savedValues` ON `savedValues`.`id`=`" + getTableName() + "`.`id` WHERE `savedValues`.`id` IS NULL";
