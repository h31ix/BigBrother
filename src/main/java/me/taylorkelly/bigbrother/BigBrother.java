...
public class BigBrother extends JavaPlugin {
    ...
    //the getInventory() method returns only half of the contents, depending on what block was right-clicked.
    //According to forums, there is no appropriate solution in bukkit API, so we must manually search, whether
    //current chest is double chest, and then eventually return merged inventories.
    public ItemStack[] getChestContents(Chest chest)
    {
    	...
    	if (second == null)	{
    		...
    		//I think it would be good, to consitently return same chest contents, regardless of what
    		//block was clicked on. That means, we must determine, which part of chest comes first, and which second.
    		//I choose the one, which has lower X coordinate. If they are same, than it's the one with lower Z coordinate.
